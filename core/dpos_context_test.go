...
func TestBlock_ElectNewDynasty(t *testing.T) {
	...
	assert.Nil(t, err)
	value, _ := util.NewUint128FromInt(2000000)
	...
	tx := NewTransaction(0, kickout, kickout, value, 1, TxPayloadDelegateType, bytes, TransactionGasPrice, gasLimit)
	_, _, err = block.executeTransaction(tx)
	assert.Nil(t, err)
	candidatePayload := NewCandidatePayload(LogoutAction)
	bytes, _ = candidatePayload.ToBytes()
	tx = NewTransaction(0, kickout, kickout, value, 2, TxPayloadCandidateType, bytes, TransactionGasPrice, gasLimit)
	_, _, err = block.executeTransaction(tx)
	assert.Nil(t, err)
	block.commit()
...
func TestTallyVotes(t *testing.T) {
	...
	assert.Nil(t, err)
	value, _ := util.NewUint128FromInt(10000)
	acc.AddBalance(value)
	dc.Accounts.Commit()
	assert.Nil(t, err)
	// empty candidates
