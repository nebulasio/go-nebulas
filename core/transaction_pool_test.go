...
func TestTransactionPool(t *testing.T) {
	...
	txs := []*Transaction{
		...
		NewTransaction(1, *other, Address{[]byte("to")}, 0, 1, []byte("dat")),
		...
		NewTransaction(1, *from, Address{[]byte("to")}, 0, 2, []byte("data")),
		NewTransaction(1, *from, Address{[]byte("to")}, 0, 3, []byte("data")),
	}
	...
	txs[0].Sign()
	assert.Nil(t, txPool.Put(txs[0]))
	// put dup tx, should fail
	assert.NotNil(t, txPool.Put(txs[0]))
	txs[1].Sign()
	assert.Nil(t, txPool.Put(txs[1]))
	txs[2].Sign()
	assert.Nil(t, txPool.Put(txs[2]))
	// put not signed tx, should fail
	assert.NotNil(t, txPool.Put(txs[3]))
	// put tx with different chainID, should fail
	txs[4].Sign()
	assert.NotNil(t, txPool.Put(txs[4]))
	// put one new, replace txs[1]
	assert.Equal(t, len(txPool.all), 3)
	assert.Equal(t, txPool.cache.Len(), 3)
	txs[6].Sign()
	assert.Nil(t, txPool.Put(txs[6]))
	assert.Equal(t, txPool.cache.Len(), 3)
	assert.Equal(t, len(txPool.all), 3)
	// get from: other, nonce: 1, data: "da"
	tx1 := txPool.Get(1, bc.tailBlock)
	assert.Equal(t, len(tx1), 1)
	assert.Equal(t, txs[2].from.address, tx1[0].from.address)
	assert.Equal(t, txs[2].nonce, tx1[0].nonce)
	assert.Equal(t, txs[2].data, tx1[0].data)
	// put one new
	assert.Equal(t, len(txPool.all), 2)
	assert.Equal(t, txPool.cache.Len(), 2)
	txs[5].Sign()
	assert.Nil(t, txPool.Put(txs[5]))
	assert.Equal(t, len(txPool.all), 3)
	assert.Equal(t, txPool.cache.Len(), 3)
	// get 2 txs, txs[5], txs[0]
	tx2 := txPool.Get(3, bc.tailBlock)
	assert.Equal(t, len(tx2), 2)
	assert.Equal(t, txs[5].from.address, tx2[0].from.address)
	assert.Equal(t, txs[5].Nonce(), tx2[0].Nonce())
	assert.Equal(t, txs[5].data, tx2[0].data)
	assert.Equal(t, txs[6].from.address, tx2[1].from.address)
	assert.Equal(t, txs[6].Nonce(), tx2[1].Nonce())
	assert.Equal(t, txs[6].data, tx2[1].data)
	// put new txs
