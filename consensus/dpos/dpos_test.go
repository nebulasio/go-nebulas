...
func TestForkChoice(t *testing.T) {
	...
	*/
	...
	block12, _ := chain.NewBlock(addr1)
	block12.SetTimestamp(BlockInterval * 2)
	block12.SetMiner(addr1)
	block12.Seal()
	am.SignBlock(addr1, block12)
	assert.Error(t, chain.BlockPool().Push(block12), core.ErrDoubleBlockMinted)

	assert.Equal(t, len(chain.DetachedTailBlocks()), 1)
	assert.Equal(t, chain.TailBlock().Hash(), block11.Hash())

	addr2 := GetUnlockAddress(t, am, "48f981ed38910f1232c1bab124f650c482a57271632db9e3")
	block111, _ := chain.NewBlockFromParent(addr2, block11)
	...
	block1111, _ := chain.NewBlockFromParent(addr3, block111)
	block1111.SetTimestamp(BlockInterval * 4)
	block1111.SetMiner(addr3)
	block1111.Seal()
	am.SignBlock(addr3, block1111)
	assert.Error(t, chain.BlockPool().Push(block1111), core.ErrMissingParentBlock)
	assert.Equal(t, len(chain.DetachedTailBlocks()), 1)
	assert.Nil(t, chain.BlockPool().Push(block111))
	assert.Equal(t, len(chain.DetachedTailBlocks()), 1)
	assert.Equal(t, chain.TailBlock().Hash(), block1111.Hash())
}
...
func TestContracts(t *testing.T) {
	...
	assert.Nil(t, err)
	block.SetConsensusState(context)
