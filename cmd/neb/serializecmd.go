...
func parseTransaction(neb *neblet.Neblet, txJSON *txJSON) (*core.Transaction, error) {
	...
	value, err := util.NewUint128FromString(txJSON.Value)
	if err != nil {
		return nil, err
	}
	gasPrice, err := util.NewUint128FromString(txJSON.GasPrice)
	if err != nil {
		return nil, err
	}
	gasLimit, err := util.NewUint128FromString(txJSON.GasLimit)
	if err != nil {
		return nil, err
	}
	...
	if txJSON.Contract != nil && len(txJSON.Contract.Source) > 0 {
		...
		payloadObj, err := core.NewDeployPayload(txJSON.Contract.SourceType, txJSON.Contract.Source, txJSON.Contract.Args)
		if err != nil {
			return nil, err
		}
		payload, err = payloadObj.ToBytes()
	} else if txJSON.Contract != nil && len(txJSON.Contract.Function) > 0 {
		payloadType = core.TxPayloadCallType

		callpayload, err := core.NewCallPayload(txJSON.Contract.Function, txJSON.Contract.Args)
		if err != nil {
			return nil, err
		}
