...
static std::string gen_admin_var(const neb::address_t &addr,
...
neb::bytes gen_auth_table_payload(const neb::address_t &nr_admin,
                                  const neb::address_t &dip_admin) {
...
TEST(test_cpp_ir, llvm_ir_content) {
  ...
  auto auth_bytes = gen_auth_table_payload(neb::bytes::from_base58(nr_base58),
                                           neb::bytes::from_base58(dip_base58));

  nbre::NBREIR ir;
  bool ret = ir.ParseFromArray(auth_bytes.value(), auth_bytes.size());
  if (!ret) {
  ...
  LOG(INFO) << "ir content size " << ir_content.size();
}
