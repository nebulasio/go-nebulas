...
func (ns *NetService) parseNebMsg(s libnet.Stream) (*Protocol, error) {
	...
	if err != nil {
		logging.VLog().Error("parse protocol, read data occurs error, ", err)
		return nil, err
	}
	protocol.data = data
...
func (ns *NetService) verifyHeader(protocol *Protocol) bool {
	...
	if !byteutils.Equal(MagicNumber, protocol.magicNumber) {
		logging.VLog().Error("verifyHeader: data verification occurs error, magic number is error, the connection will be closed.")
		return false
	}

	if node.Config().ChainID != byteutils.Uint32(protocol.chainID) {
		logging.VLog().Error("verifyHeader: data verification occurs error, chainID is error, the connection will be closed.")
		return false
	}

	if node.version != protocol.version {
		logging.VLog().Error("verifyHeader: data verification occurs error, version is error, the connection will be closed.")
		return false
	}

	if dataHeaderChecksum != byteutils.Uint32(protocol.headerChecksum) {
		logging.VLog().Error("verifyHeader: data verification occurs error, dataHeaderChecksum is error, the connection will be closed.")
		return false
	}
	return true
}
