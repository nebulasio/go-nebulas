...
//node tps.test.js testneb1 8000 1
...
//env = 'local';  //just used for local test
//AddressNumber = 3;
//SendTimes = 2;
...
console.log("to address:" , to.getAddressString());
var lastnonce = 0;
...
function sendTransaction(index, totalTimes, from, to, value, nonce) {
    if (index < totalTimes) {
        ...
        neb.api.sendRawTransaction(rawTx).then(function (resp) {
            console.log("send raw transaction resp:" + JSON.stringify(resp)+ "from:" + from.getAddressString() + ", to:" + to.getAddressString());
            if (resp.txhash) {
